...
public static class MvcExtensionMethods
{
    ...
    {
        var output = @"<link type=""{0}"" rel=""edituri"" title=""{1}"" href=""http://{2}"" />";
    ...
    {
        TagBuilder builder = new TagBuilder("a") {InnerHtml = !string.IsNullOrEmpty(linkText) ? HttpUtility.HtmlEncode(linkText) : string.Empty};
